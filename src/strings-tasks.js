...
function isPalindrome(str) {
  ...
  const reg = /,\s|\s/;
  let startsStr = str.split(reg).join('');
  if (lastSymbol === '!' || lastSymbol === '?') {
    startsStr = startsStr.slice(0, -1);
  }
  const reverseStr = startsStr.split('').reverse().join('');
  return startsStr.toUpperCase() === reverseStr.toUpperCase();
}
...
function invertCase(str) {
  ...
  return str.replace(/[a-zA-Z]/g, changeCase);
}
...
function encodeToRot13(str) {
  ...
  return str.replaceAll(/[a-zA-Z]/g, (match) =>
    cipherOut.charAt(cipherIn.indexOf(match))
  );
}
